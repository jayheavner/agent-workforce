...
Split for the project's file-size discipline, and sourcing this file is still the
whole hook: `agent_team_closeout_paths.py` holds what git says changed and which
of it is code, and `agent_team_closeout_ledger.py` holds the delivery ledger's
checks. What stays here is the Stop event itself — transcript scan, cost report,
enforcement state, telemetry, and the block/allow decision.
"""
import hashlib
import json
import os
...
if HERE not in sys.path:                      # the two halves live beside this file
    sys.path.insert(0, HERE)

from agent_team_closeout_paths import git_dirty                    # noqa: E402
from agent_team_closeout_ledger import (MUTATING_ROLES,            # noqa: E402
                                        ledger_checks)
...
def main():
    ...
    def build_missing(tail):
        ...
        found.extend(ledger_checks(tail, roles, order, cwd, session_id))
