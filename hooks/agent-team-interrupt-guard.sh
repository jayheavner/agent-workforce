...
printf 'agent-team interrupt guard: the %s dispatch returned without a WORKFORCE_REPORT marker in its final lines — treat it as interrupted (turn cap, context exhaustion, or crash), NOT as a completed phase and NOT as a blocker report. Do not advance the route on its partial output. Reconcile and resume: (1) inspect the workspace read-only — git log/status for its commits, test state, files touched, AND run `python3 $HOME/.claude/hooks/agent_team_dispatch_recap.py --transcript <its transcript path>` to get what it had already found, not a guess at it — a resumed dispatch handed only a prose summary has been measured, this session, re-covering 61%% of a predecessor'"'"'s files and running its exact commands over again before running out of its own turns; (2) re-dispatch the same role with the recap'"'"'s output, the remaining ACCEPTANCE CRITERIA, and the word RESUME in the prompt; (3) after two interrupted resumes of the same phase, stop and escalate to the human with the evidence.\n' "$ROLE" >&2
exit 2
