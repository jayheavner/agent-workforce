...
Per `policy:workspace-isolation`, the unit of isolation is the **change**, not the agent: a dispatch declares it on a line reading `CHANGE: <slug>`, and the dispatch guard claims that change in the work register and creates or adopts its worktree before the agent's first turn. The path is derived from the slug and never passed — `<project>/.claude/worktrees/<slug>`, on the ref `refs/heads/change/<slug>`. Every phase of that work — architect, test-author, builder, verifier, reviewer, deployer, scribe, executor — declares the same slug and works inside that one workspace, never the shared checkout and never another change's tree.
