...
Give every specialist complete context: objective, tier, workspace, relevant artifacts, applicable role contract, mutation boundary, and the exact deliverable the next phase needs. Workspace means one explicit path per specialist: each builder gets its own unique worktree (it creates one before touching code and reports the path — see `policy:workspace-isolation`), never a path shared with another builder, and each verifier, reviewer, or deployer gets the worktree path its builder reported. Builders on disjoint files run concurrently. Ask the specialist to read only the relevant section of [roles.md](references/roles.md) plus any task-specific bundled skills.
