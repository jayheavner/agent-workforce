...
Author every builder phase's `ACCEPTANCE CRITERIA` block BEFORE the build — from the architect's plan when one ran, from the debugger's `REPRO COMMAND:` line when the route is a symptom repair, from the human's request in its own terms otherwise — in the linted criterion shape (`- [ ] AC-N (mechanical): <claim>. Check: `<command that can fail and prints why>` -> expects <observable>.` or `- [ ] AC-N (judgment): <claim>. Judge: <who>. Bar: <what a "no" looks like>.`), and give the verifier the identical block verbatim. The builder's own tests are never the bar: whoever writes the code is never the last author of what "done" means. The criteria block is the task's completion ledger; every completion claim traces to it.
