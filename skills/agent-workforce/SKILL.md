...
Route symptom-shaped requests — broken behavior, a failing test, an unexpected result, or "why is this wrong" — to `agent_workforce_debugger` before assigning a build tier or a builder. The debugger diagnoses and returns evidence; it does not apply a fix. Its `REPRO COMMAND:` line becomes the Check of AC-1 in the acceptance block you author, quoted exactly, and that same AC-1 goes to the builder phase, the executor phase when the fix is trivial enough to route there, and the verifier phase; when the debugger reports `none` instead, author that criterion yourself and say so. Use `agent_workforce_debugger_deep` for the second diagnosis of the same symptom or for a cross-system failure. Only after a diagnosis identifies a repair should the normal architecture/build route begin.
...
Give every specialist complete context: objective, tier, workspace, relevant artifacts, applicable role contract, mutation boundary, and the exact deliverable the next phase needs. Workspace means one declared change per writing dispatch: the prompt carries a line reading `CHANGE: <slug>`, and the dispatch guard claims that change in the work register and creates or adopts its worktree at the path derived from the slug (see `policy:workspace-isolation`) — you pass the slug, never a path. Every phase of one piece of work reuses that slug and so lands in that one workspace; two different changes run concurrently, while one change takes its writing turn one dispatch at a time. Ask the specialist to read only the relevant section of [roles.md](references/roles.md) plus any task-specific bundled skills.

Author every builder phase's `ACCEPTANCE CRITERIA` block BEFORE the build — from the architect's plan when one ran, from the debugger's `REPRO COMMAND:` line when the route is a symptom repair, from the human's request in its own terms otherwise — in the linted criterion shape (`- [ ] AC-N (mechanical): <claim>. Check: `<command that can fail and prints why>` -> expects <observable>.` or `- [ ] AC-N (judgment): <claim>. Judge: <who>. Bar: <what a "no" looks like>.`), and give the verifier the identical block verbatim. When that `REPRO COMMAND:` line carries the literal ` [throwaway]` marker, author AC-1's Check as a durable equivalent that will still exist and run when the verifier needs it, and say in the phase dispatch that you did so. The builder's own tests are never the bar: whoever writes the code is never the last author of what "done" means. The criteria block is the task's completion ledger; every completion claim traces to it.
