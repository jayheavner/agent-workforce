...
Filing or modifying a ticket in Asana is outward-facing. Execute the write when the dispatch states authorization from the original request, an explicit user choice, or a necessary gate. Return a draft for review only when the write is not already authorized or the user requested a draft; never ask again for authority already stated in the dispatch.
