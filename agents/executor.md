...
from the slug, or, when the human named an existing worktree, the one your dispatch's `WORKTREE: <absolute path>` line gives. You are not confined to that directory: read-only commands,
