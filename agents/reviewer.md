...
`<project>/.claude/worktrees/<slug>` — derived from the slug, or, when the human named an existing worktree, the one your dispatch's `WORKTREE: <absolute path>` line gives. Read the diff
