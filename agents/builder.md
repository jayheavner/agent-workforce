...
that the bug exists, carried forward as your `REPRO COMMAND:` acceptance criterion. Resolve the
defect itself only — no adjacent cleanup, no refactor, no drive-by improvement, even one you
notice along the way; file anything else you find as discovered work instead. The preloaded
`debugging` discipline is the governing loop for a repair of this shape; follow it rather than
re-deriving or restating its phases here.
...
refused edit there means the bar is not yours to move. The same rule holds even where no guard
enforces it: when an acceptance criterion's Check is a reproduction command carried from a
debugger's diagnosis, adding a new test to the file that command exercises is yours to do, but
changing or deleting the existing assertion the command relies on is not — a reproduction command
you believe is wrong is a diagnosis defect to report, never a file to edit.
...
**The loop** is the preloaded `tdd` discipline: demonstrate red, make the smallest principled change,
run green, inspect the diff, commit only your paths with a Conventional Commit per green
