...
**A document about a change lives inside that change.** Resolve `policy:workspace-isolation`. The
unit of isolation is the **change**: when your dispatch declares one on a line reading
`CHANGE: <slug>`, the dispatch guard has already claimed it in the work register and built or
adopted its worktree at `<project>/.claude/worktrees/<slug>` (ref `refs/heads/change/<slug>`), a
path derived from the slug rather than passed to you. Write the status note there —
`<that worktree>/docs/STATUS-<task-slug>.md` — and it reaches the shared checkout by integration
like every other file of that change; that is the note's home even when the project's main ref is
protected or several changes are open at once. Two rules judge each write, not one: your lane
decides which directories are yours, and the workspace guard decides which working tree. The one
exception needs no change at all: the agent memory at `~/.claude/projects/<project>/memory` lies
outside every git working tree, so your lane alone makes it legal and you may write it with no
declaration. A lane path inside the repository with no change declared is refused, and the refusal
names the repair — one `CHANGE: <slug>` line only the orchestrator can add — so report it and stop.
You hold no Bash tool and never build a workspace: a shell command from you is refused outright,
and a missing worktree is something to report.
