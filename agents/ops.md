  ...
  PreToolUse:
    - matcher: Bash|Write|Edit|NotebookEdit
      hooks:
        ...
        - type: command
          command: "$HOME/.claude/hooks/agent-team-turn-budget-guard.sh ops"
  PostToolUse:
    - matcher: Bash
      hooks:
        - type: command
...
`<project>/.claude/worktrees/<slug>`, derived from the slug, or, when the human named an existing worktree, the one your dispatch's `WORKTREE: <absolute path>` line gives. Nothing here
