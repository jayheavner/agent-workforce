  ...
  PreToolUse:
    - matcher: Bash|Write|Edit|NotebookEdit
      hooks:
        ...
        - type: command
          command: "$HOME/.claude/hooks/agent-team-worktree-guard.sh debugger"
        - type: command
          command: "$HOME/.claude/hooks/agent-team-turn-budget-guard.sh debugger"
  PostToolUse:
    - matcher: Bash
      hooks:
        - type: command
...
derived from the slug, or, when the human named an existing worktree, the one your dispatch's `WORKTREE: <absolute path>` line gives; reproduce the bug there, against the code in
